...
dt = pickle.load(open('pt1.pkl','rb'))  # rb = read binary
...
st.title('ME/CFS and Depression Prediction')
...
with col1:
  ...
  bf_level = st.number_input('Brain Fog Level',1.0,10.0,2.0)
  pps_score = st.number_input('Physical Pain Score',1.0,10.0,4.5)
...
with col2:
  ...
  sleep_hrs = st.number_input('Sleep Hrs',3.0,10.0,6.4)
...
test_input = [[age,gen_m, sq_index,bf_level,pps_score, stress_level,dep_phq9,
              fs_scale,pem_dur,sleep_hrs,pem_y,med_y,ws_pw,ws_w,
              sl_l,sl_m,sl_vh,sl_vl,ex_n,ex_of,ex_r,ex_s]]
